...
public class RegisterCarArrivedBehaviour extends CyclicBehaviour {
	...
	public void action() {
		...
		if (msg != null) {
			...
			System.out.println(myAgent.getLocalName() + ": Car arrived."
					+ "Total " + trafficLightAgent.getWaitingCarsCount());
			if (trafficLightAgent.getWaitingCarsCount() >=
					trafficLightAgent.getMaxCarsWaitingCount() 
		...
		private ACLMessage prepareMessage() {
			...
			try {
				msg.setContentObject(new SwitchColorPropose(LightsColor.Green,
						((TrafficLightAgent) myAgent).getPriority()));
			} catch (IOException e) {
				e.printStackTrace();
			}
